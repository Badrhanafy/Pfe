                ...
                <div class="flex gap-3 flex-wrap">
                    <form method="GET" action="{{ route('Artisans') }}" class="flex gap-3 flex-wrap">
                        ...
                        <select name="sort" class="rounded-lg border border-gray-300 px-4 py-2">
                            ...
                            <option value="most_experience" {{ request('sort') == 'most_experience' ? 'selected' : '' }}>Most Experience</option>
                        </select>
            ...
            <div class="bg-white rounded-xl shadow overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Name</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6 hidden md:table-cell">Profession</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Location</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Experience</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Rating</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6 hidden sm:table-cell">Joined</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-4 whitespace-nowrap sm:px-6">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        <div class="text-sm text-gray-500 md:hidden">{{ $artisan->profession }}</div>
                                        <div class="text-sm text-gray-500 sm:hidden">{{ $artisan->email }}</div>
                                    </div>
                                </div>
                            </td>
                            
                            <!-- Profession Column -->
                            <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500 sm:px-6 hidden md:table-cell">
                                {{ $artisan->profession }}
                            </td>
                            ...
                            <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500 sm:px-6">
                                @if($artisan->reviews_avg_rating)
                                    <span class="{{ $artisan->reviews_avg_rating >= 4 ? 'text-green-600' : ($artisan->reviews_avg_rating >= 2.5 ? 'text-yellow-600' : 'text-red-600') }}">
                            ...
                            <!-- Joined Date Column -->
                            <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500 sm:px-6 hidden sm:table-cell">
                            ...
                            <td class="px-4 py-4 whitespace-nowrap text-sm font-medium sm:px-6">
                                <div class="flex space-x-4">
                                    <button onclick="openModal()" class="text-blue-600 hover:underline">Edit</button>
                        ...
                        @empty
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-gray-500">No artisans found</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
...
<div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
        ...
        <p class="mb-4 text-gray-600">Are you sure you want to delete <span id="artisanName" class="font-bold"></span>?</p>
...
<!-- Modal Container -->
<div id="updateArtisanModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden items-center justify-center">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-4xl mx-auto overflow-hidden">
      ...
      <form method="POST" action="{{ route('update.artisan') }}" enctype="multipart/form-data" class="flex flex-col md:flex-row p-6 space-y-4 md:space-y-0 md:space-x-6">
        @csrf
        @method('PATCH')
        <input type="hidden" name="id" id="update-artisan-id">
  
        <!-- Left: Profile image -->
        <div class="md:w-1/3 flex flex-col items-center justify-start">
          <img id="preview-img" src="{{ $artisan->photo ? asset('storage/' . $artisan->photo) : asset('images/default-avatar.png') }}" alt="Photo Preview" class="w-32 h-32 object-cover rounded-full border-4 border-blue-200 mb-4">
        ...
        <div class="md:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="name" value="{{ $artisan->name }}" id="update-artisan-name" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
          </div>
          <div>
            <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
            <input type="email" name="email" value="{{ $artisan->email }}" id="update-artisan-email" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
          </div>
          <div>
            <label for="profession" class="block text-sm font-medium text-gray-700">Profession</label>
            <input type="text" name="profession" value="{{ $artisan->profession }}" id="update-artisan-profession" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
          </div>
          <div>
            <label for="address" class="block text-sm font-medium text-gray-700">Location</label>
            <input type="text" name="address" value="{{ $artisan->address }}" id="update-artisan-address" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
          </div>
          <div class="col-span-2">
            <label for="experience_years" class="block text-sm font-medium text-gray-700">Experience (years)</label>
            <input type="number" name="experience_years" value="{{ $artisan->experience }}" id="update-artisan-experience" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
          </div>
        </div>

        <script>
            function openModal() {
              document.getElementById('updateArtisanModal').classList.remove('hidden');
              document.getElementById('updateArtisanModal').classList.add('flex');
            }
          
            function closeModal() {
              document.getElementById('updateArtisanModal').classList.remove('flex');
              document.getElementById('updateArtisanModal').classList.add('hidden');
            }
      ...
      <div class="flex justify-end space-x-3 p-4 bg-gray-100 border-t">
        <button onclick="closeModal()" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400">Cancel</button>
        <button onclick="document.querySelector('form').submit()" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Save</button>
      </div>
...
<script>
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            deleteForm.action = `/admin/artisans/${artisanId}`; // غيّر هاد المسار إلا كنتي كاتب route مختلف
